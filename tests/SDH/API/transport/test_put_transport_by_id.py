...
@allure.feature('transport put by id')
...
@allure.title("Запрос на обновление transport с корректными параметрами")
def test_all_valid_parameters(headers,endpoints,body,status_code):
    id=endpoints["id"]
    url=f"{URL}/{PATH}/{id}/"
...
@allure.title("Запрос на обновление transport с некорректным headers")
def test_invalid_header(headers,endpoints,body,status_code):
    id=endpoints["id"]
    url=f"{URL}/{PATH}/{id}/"
...
@allure.title("Запрос на обновление transport с  некорректным endpoint ")
def test_invalid_endpoint(headers,endpoints,body,status_code):
    id=endpoints["id"]
    url=f"{URL}/{PATH}/{id}/"
...
@allure.title("Запрос на обновление transport с некорректными параметрами body")
def test_no_valid_body(headers,endpoints,body,status_code):
    id=endpoints["id"]
    url=f"{URL}/{PATH}/{id}/"
    data=json.dumps(body)
    response = requests.request(method=METHOD,url=url, headers=headers ,data=data)
...
@allure.title("Запрос на обновление transport с корректными параметрами, проверка схемы ответа")
def test_response_scheme(headers,endpoints,body,status_code,response_schema):
    id=endpoints["id"]
    url=f"{URL}/{PATH}/{id}/"
