...
################################################################
@fixture
@parametrize("id",[EXISTS_TRANSPORT_ID])
...
################################################################
@fixture
@parametrize("id",[NO_EXISTS_TRANSPORT_ID])
...
@parametrize("protocol",[VALID_STRING(15)])
@parametrize("status",[VALID_STRING(15),VALID_OPTIONAL])
def valid_extended_body(name,description, protocol,status):
    ...
    if not isinstance(protocol,OptionalFields):
...
@parametrize("name",[VALID_STRING(40)])
@parametrize("protocol",[VALID_STRING(15)])
def valid_optional_body(name, protocol):
...
@parametrize("protocol",[VALID_OPTIONAL,VALID_STRING(15+1)])
@parametrize("status",[VALID_NUMBER_INT,VALID_STRING(15+1)])
def no_valid_body(name,description, protocol,status):
...
@allure.feature('transport post')
@allure.suite("check body")
@allure.story('positive')    
@allure.story('response 201') 
@allure.testcase("https://cdp.colvir.ru/wiki/display/SDH/A000002", 'A000002')
@allure.description("""
     Запрос на создание transport с корректными параметрами 
     Ожидаемый результат: response.status_code=201 """)
@parametrize("headers",[valid_headers]) 
@parametrize("body",[valid_extended_body]) 
@parametrize("status_code",[RESPONSE_CREATED]) 
@allure.title("Запрос на создание transport с корректными параметрами")
def test_all_valid_parameters(headers,body,status_code):

    url=f"{URL}/{PATH}/"

    response = requests.request(method=METHOD,url=url, headers=headers ,json=body)

    Attach(url=url,headers=headers,body=body,response=response.json()).attach() 
...
@allure.description(""" 
    Запрос на создание transport с некорректным headers
    Ожидаемый результат: response.status_code=415
                    """)
@parametrize("headers",[no_valid_headers]) 
@parametrize("body",[valid_optional_body]) 
@parametrize("status_code",[RESPONSE_UNSUPPORTED]) 
@allure.title("Запрос на создание transport с некорректным headers")
def test_invalid_header(headers,body,status_code):
...
@allure.suite("check body")
@allure.story('negative')  
@allure.story('response 400')  
@allure.testcase("https://cdp.colvir.ru/wiki/display/SDH/A000002", 'A000002')
@allure.description("""
    Запрос на создание transport с некорректным body
    Ожидаемый результат: response.status_code=400
    """)
@parametrize("headers",[valid_headers]) 
@parametrize("body",[no_valid_body]) 
@parametrize("status_code",[RESPONSE_BAD_REQUEST]) 
@allure.title("Запрос на создание transport с некорректными параметрами body")
def test_no_valid_body(headers,body,status_code):
...
@allure.title("Запрос на создание transport с корректными параметрами, проверка схемы ответа")
def test_response_scheme(headers,body,status_code,response_schema):
